...
export const getLevelFromXp = (xp) => {
  const n = typeof xp === 'number' && !isNaN(xp) ? Math.max(0, xp) : 0;
  for (let i = 0; i < XP_CUMULATIVE.length; i++) {
    if (n < XP_CUMULATIVE[i]) return i + 1; // levels 1–19
  }
  return 20; // max
};
...
export const getXpInfo = (xp) => {
  const n = typeof xp === 'number' && !isNaN(xp) ? Math.max(0, xp) : 0;
  const level = getLevelFromXp(n);
  if (level >= 20) {
    return { level: 20, xpInLevel: XP_THRESHOLDS[18], xpToNext: 0, progressPct: 100 };
  }
  const prevCum = level >= 2 ? XP_CUMULATIVE[level - 2] : 0;
  const xpInLevel = n - prevCum;
  const xpToNext = XP_THRESHOLDS[level - 1];
  return { level, xpInLevel, xpToNext, progressPct: Math.min(100, Math.floor((xpInLevel / xpToNext) * 100)) };
};

export const getLevelCharacter = (xp) => {
...
/** Maps XP level to in-game player tier (1–5). */
export const getGamePlayerLevel = (xp) => {
  const { level } = getXpInfo(xp);
